...
function* registerProspect(action) {
  try {
    const payload = yield call(api.registerProspect, action);
    console.log('payload', payload);
    yield put({ type: constants.ACCOUNT_REGISTER_PROSPECT.SUCCESS, payload });
    yield call(history.push, '/lz/register?ok');
  } catch (e) {
    yield put({
      type: constants.ACCOUNT_REGISTER_PROSPECT.FAILED,
      message: e.message || e
    });
  }
}

function* watchRegisterProspect() {
  yield takeLatest(constants.ACCOUNT_REGISTER_PROSPECT.ACTION, registerProspect);
}
